...
namespace Shaml.Membership.OpenIDMembershipProvider
{
    public class PgOpenIDMembershipProvider : MembershipProvider
    {
        ...
        public string GetUserNameByOpenID(string id)
        {
            ...
            using (NpgsqlConnection dbConn = new NpgsqlConnection(m_connectionString))
            {
                using (NpgsqlCommand dbCommand = dbConn.CreateCommand())
                {
                    dbCommand.CommandText = string.Format(CultureInfo.InvariantCulture, "SELECT \"UserName\" FROM \"OpenIDAccounts\" WHERE \"OpenIDString\" = @OpenIDString AND  \"ApplicationName\" = @ApplicationName", s_tableName);
                    ...
                    try
                    {
                        ...
                        string uid = (string)dbCommand.ExecuteScalar();
                        if (uid != null) {
                            u = uid;
                        }
                    }
        ...
        public List<string> GetOpenIDIdentifiersForUserName(string UserName)
        {
            ...
            using (NpgsqlConnection dbConn = new NpgsqlConnection(m_connectionString))
            {
                using (NpgsqlCommand dbCommand = dbConn.CreateCommand())
                {
                    dbCommand.CommandText = string.Format(CultureInfo.InvariantCulture, "SELECT \"OpenIDString\" FROM \"OpenIDAccounts\" WHERE \"UserName\" = @UserName AND \"ApplicationName\" = @ApplicationName");
